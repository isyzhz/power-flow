function [V, converged, i] = CJ3_Vtemp(Ybus, Sbus, V0, ref, pv, pq, mpopt)
...
while (~converged && i < max_it)
    ...
    if normP < tol && normQ < tol
        converged = 1;
        if mpopt.verbose
            fprintf('\nCJ3-Vtemp converged in %d iterations.\n', i);
        end
        break;
    end
end
...
if mpopt.verbose
    if ~converged
        fprintf('\nCJ3-Vtemp did not converge in %d iterations.\n', i);
    end
end
